...
function wcpl_gp_param_isset_assign($method, $key, &$param)
{
    isset($method[$key]) ? $param[$key] = $method[$key] : '';
}
...
function wcpl_paylane_js_validation()
{
    ...
    echo <<<EOF
<script>
	window.PAYLANE_VALIDATION_MESSAGES = $json
</script>
EOF;
...
function wcpl_init_paylane()
{
    ...
    if (!function_exists('wcpl_get_errors_by_code')) {
        require_once __DIR__ . '/includes/paylane-errors.php';
    }

    if (!class_exists('WCPL_Logger')) {
    ...
    class WC_Gateway_Paylane extends WC_Payment_Gateway
    {
         ...
         * @var string
         */
        const PAYMENT_METHOD_BLIK0 = 'blik0';
        ...
        public function paylane_payment_style()
        {
            if (is_checkout()) {
                wp_register_style(
                    ...
                    ), [], '234_' . $this->get_option('design'), 'all'
                );
        ...
        {
            $communication_id = sanitize_text_field(wcpl_gp_param_isset($_POST, 'communication_id'));
            $token = sanitize_text_field(wcpl_gp_param_isset($_POST, 'token'));
            ...
            if (isset($_POST['content']) && ($this->enable_notification === 'yes')) {
                ...
                try
                {
                    $this->handle_notification($token, $communication_id);
                } catch (Exception $e) {
            ...
            $type = sanitize_text_field(wcpl_gp_param_isset($_GET, 'type'));

            if (!$type) {
                $this->response_check();
            } else {
                $order_id = sanitize_text_field(wcpl_gp_param_isset($_GET, 'order_id'));
                        ...
                        switch ($type) {
                            case WC_Gateway_Paylane::PAYMENT_METHOD_CREDIT_CARD:
                                ...
                                if ($client->isSuccess()) {
                                    ...
                                    $error_message = '';
                                    $resp = array(
                                        'id' => $result['id_3dsecure_auth'],
                                        'description' => $order_id,
                                    );

                                    list($error_message, $response) = $this->cardPay($client, $resp, $error_message);

                                    $response['status'] = 'PENDING';
                                    $response['description'] = $order_id;
                                    $this->cardPaySaleCheck($error_message, $type, $response, false);
                                    exit;
                    ...
                    if ($client->isSuccess()) {
                        ...
                        switch ($type) {
                            case WC_Gateway_Paylane::PAYMENT_METHOD_CREDIT_CARD:
                                ...
                                $this->finish_order($order_id, WC_Gateway_Paylane::ORDER_STATUS_PENDING); //$this->get_option('status_successful_order')
                                break;
                            ...
                            case WC_Gateway_Paylane::PAYMENT_METHOD_BLIK0:
                                $blikStatus = $this->checkBlikStatus($order_id);
                                if ($blikStatus !== 'S') {
                                    $error_message = __('Payment Error Occurred', 'wc-gateway-paylane');
                                    ...
                                    WCPL_Logger::log("[finish_order]\nTransaction, payment method: " . $payment_label . "\norder_id: " . $order_id);
        ...
        function response_check()
        {
            if (isset($_POST['description'])) {
                $order_id = sanitize_text_field(wcpl_gp_param_isset($_POST, 'description'));
            } else {
                $order_id = sanitize_text_field(wcpl_gp_param_isset($_GET, 'description'));
            }
            ...
            $_data_method = array();
            if ((isset($_GET['status']) && isset($_GET['hash'])) || (empty($_POST) && isset($_GET['correct']) && isset($_GET['id_sale']))) {
                //mamy GET

                $response['status'] = $this->checkRequestStatus(wcpl_gp_param_isset($_GET, 'status'));
                $response['description'] = sanitize_text_field(wcpl_gp_param_isset($_GET, 'description'));
                $response['amount'] = wc_format_decimal(wcpl_gp_param_isset($_GET, 'amount'));
                $response['currency'] = $this->checkCurrency(wcpl_gp_param_isset($_GET, 'currency'));
                $response['hash'] = sanitize_text_field(wcpl_gp_param_isset($_GET, 'hash'));

                wcpl_gp_param_isset_assign($_GET, 'id_error', $_data_method);
                wcpl_gp_param_isset_assign($_GET, 'error_code', $_data_method);
                wcpl_gp_param_isset_assign($_GET, 'error_text', $_data_method);
                wcpl_gp_param_isset_assign($_GET, 'id_sale', $_data_method);

                $id_sale = sanitize_text_field(wcpl_gp_param_isset($_GET, 'id_sale'));
                $id_error = sanitize_text_field(wcpl_gp_param_isset($_GET, 'id_error'));
                $error_code = sanitize_text_field(wcpl_gp_param_isset($_GET, 'error_code'));
                $error_text = sanitize_text_field(wcpl_gp_param_isset($_GET, 'error_text'));
            } else {
                //mamy POST

                $response['status'] = $this->checkRequestStatus(wcpl_gp_param_isset($_POST, 'status'));
                $response['description'] = sanitize_text_field(wcpl_gp_param_isset($_POST, 'description'));
                $response['amount'] = wc_format_decimal(wcpl_gp_param_isset($_POST, 'amount'));
                $response['currency'] = $this->checkCurrency(wcpl_gp_param_isset($_POST, 'currency'));
                $response['hash'] = sanitize_text_field(wcpl_gp_param_isset($_POST, 'hash'));

                wcpl_gp_param_isset_assign($_POST, 'id_error', $_data_method);
                wcpl_gp_param_isset_assign($_POST, 'error_code', $_data_method);
                wcpl_gp_param_isset_assign($_POST, 'error_text', $_data_method);
                wcpl_gp_param_isset_assign($_POST, 'id_sale', $_data_method);

                $id_sale = sanitize_text_field(wcpl_gp_param_isset($_POST, 'id_sale'));
                $id_error = sanitize_text_field(wcpl_gp_param_isset($_POST, 'id_error'));
                $error_code = sanitize_text_field(wcpl_gp_param_isset($_POST, 'error_code'));
                $error_text = sanitize_text_field(wcpl_gp_param_isset($_POST, 'error_text'));
            }
                ...
                if ($hash == $response['hash']) {
                    if ($response['status'] != 'ERROR') {
                        if ($response['status'] == 'PENDING') {
                            ...
                            @session_start();
                            $this->finish_order($response['description'], WC_Gateway_Paylane::ORDER_STATUS_PENDING); //$this->get_option('status_successful_order')
                            WCPL_Logger::log("[response_check]\nFinish order: " . $response['description'] . "\norder_id: " . $order_id . "\nsale_id: " . $response['id_sale']);
                        }
                    }
                } else {
        ...
        function response_check_3ds()
        {
            ...
            $response['hash'] = (wcpl_gp_param_isset($_GET, 'hash'));
            if (is_null($response['hash'])) {
                ...
                if ($response['status'] !== 'ERROR') {
                    $response['id'] = sanitize_text_field(wcpl_gp_param_isset($_POST, 'id_3dsecure_auth'));
                } else {
                    $error_message .= " Error: " . esc_attr($response['status']);
                }
            } else {
                //mamy GET
                $response['hash'] = sanitize_text_field($response['hash']);

                $response['status'] = sanitize_text_field(wcpl_gp_param_isset($_GET, 'status'));
                $response['description'] = sanitize_text_field(wcpl_gp_param_isset($_GET, 'description'));
                $response['amount'] = wc_format_decimal(wcpl_gp_param_isset($_GET, 'amount'));
                $response['currency'] = $this->checkCurrency(wcpl_gp_param_isset($_GET, 'currency'));

                if ($response['status'] !== 'ERROR') {
                    $response['id'] = sanitize_text_field(wcpl_gp_param_isset($_GET, 'id_3dsecure_auth'));
                } else {
                    $error_message .= " Error: " . esc_attr($response['status']);
                }
            }
            ...
            if ($response['status'] === 'ERROR' || $error_message != "") {
                ...
                list($error_message, $response2) = $this->cardPay($client, $response, $error_message);
            }

            //todo - sprawdzic czy dziala normalny flow
            $notificationType = get_post_meta($order_id, 'paylane-notification-type', true);

            if ($notificationType === false || ($notificationType !== 'S')) {
                $this->cardPaySaleCheck($error_message, $type, $response);
            }

            exit;
        }

        protected function cardPay($client, $response, $error_message)
        {
            try
            {
        ...
        protected function cardPaySaleCheck($error_message, $type, $response, $checkHash = true)
        {
            if ($error_message == "") {
                if ($checkHash) {
                ...
                if (($checkHash && $hash == $response['hash']) || !$checkHash) {
                    if ($response['status'] != 'ERROR') {
                        if ($response['status'] == 'PENDING') {
                            $this->finish_order(
                                $response['description'],
        ...
        {
            if (!class_exists('Paylane_Gateway_Base')) {
        ...
        {
            if (!class_exists('Paylane_Gateway_Secure')) {
        ...
        {
            if (!class_exists('Paylane_Gateway_CreditCard')) {
                require_once __DIR__ . '/Gateway/CreditCard.php';
            }
            if (!class_exists('Paylane_Gateway_BankTransfer')) {
                require_once __DIR__ . '/Gateway/BankTransfer.php';
            }
            if (!class_exists('Paylane_Gateway_Ideal')) {
                require_once __DIR__ . '/Gateway/Ideal.php';
            }
            if (!class_exists('Paylane_Gateway_Paypal')) {
                require_once __DIR__ . '/Gateway/Paypal.php';
            }
            if (!class_exists('Paylane_Gateway_Sepa')) {
                require_once __DIR__ . '/Gateway/Sepa.php';
            }
            if (!class_exists('Paylane_Gateway_Sofort')) {
                require_once __DIR__ . '/Gateway/Sofort.php';
            }
            if (!class_exists('Paylane_Gateway_ApplePay')) {
                require_once __DIR__ . '/Gateway/ApplePay.php';
            }
            if (!class_exists('Paylane_Gateway_Blik0')) {
                require_once __DIR__ . '/Gateway/Blik0.php';
            }
        ...
        function sanitize_notification($d)
        {
            return $d;
        }
         ...
         */
        function handle_notification($token, $communication_id)
        {
        ...
        private function parseNotification($notification, $order)
        {
            ...
            if ($notificationType === false || ($notificationType !== false && $this->canUpdateStatus($notificationType, $notification['type']))) {
                ...
                if ($notification['type'] === 'S') {
                    $paylane_code = get_post_meta($order->get_id(), 'paylane-type', true);
                    $paylane_methods = self::$paylane_methods;
                    $payment_label = $paylane_methods[$paylane_code];
                    ...
                    $order->update_status($this->getCorrectOrderStatus($this->get_option('status_successful_order')), 'Polskie ePłatności: ' . $order_status_message);
                    // $order->update_status($this->get_option('status_successful_order'), 'Polskie ePłatności: ' . __('Transaction complete', 'wc-gateway-paylane'));
        ...
        {
            $cs = get_woocommerce_currencies();
            if (isset($cs[$currency])) {
                return $currency;
            }
            return null;
        }

        private function checkBlikStatus($orderId)
        {
            global $wpdb;
            $i = 0;
            while (1) {
                ...
                $status = $wpdb->get_var("select meta_value from `" . $wpdb->postmeta . "` where post_id = '" . $orderId . "' and meta_key = 'paylane-notification-type'");
